...
1.1.6 Direct Disbursement of Rent. Rental proceeds for each stay are paid by the Booking Site directly to Subscriber, after the Booking Site deducts its own charges (the "Platform Payout"). Those Booking Site deductions may include taxes, merchant or processing fees, and the platform's booking percentage. Host does not collect, hold, or distribute those proceeds. The Platform Payout is not Gross Rental Income. Host's Subscription Fee is invoiced under Section 2.2 on Gross Rental Income, which includes the platform fee and is not reduced by what the Booking Site withheld.

1.1.7 Reporting. Host agrees to prepare monthly reports to Subscriber detailing Gross Rental Income (the base for the Subscription Fee), the Platform Payout, Booking Site deductions, and costs and expenses incurred by Host on Subscriber's behalf. Subscriber will be provided with access to those reports by email or file-sharing folder. Those reports will be made available, with respect to each month, on or before the 15th day of the next succeeding month.
...
2.2 Subscription Fee. The Subscription Fee is fifteen percent (15%) of Gross Rental Income. "Gross Rental Income" means the Accommodation fare, Extra person fees, Weekly discounts (as they affect the Accommodation fare), any markup or markdown Host applies to the Accommodation fare (including a markup to offset Booking Site / platform fees), and Booking Site / platform fees (including host service fees and similar platform charges). The Subscription Fee is calculated on Gross Rental Income without first subtracting the platform fee. Gross Rental Income does not include cleaning fees or taxes. Host does not charge the Subscription Fee on the cleaning fee. Gross Rental Income also does not include other fees allocated one hundred percent (100%) to Host under the parties' PMS commission settings (including, without limitation, early check-in fees and Electronics Fees). A markup Host adds to the Accommodation fare to offset platform fees is Gross Rental Income; it is not an excluded PMS fee.
