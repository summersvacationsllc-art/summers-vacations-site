...
1.1.6 Direct Disbursement of Rent. Rental proceeds for each stay are paid by the Booking Site directly to Subscriber, after the Booking Site deducts its own charges (the "Platform Payout"). Those Booking Site deductions may include taxes, merchant or processing fees, and the platform's booking percentage. Host does not collect, hold, or distribute those proceeds. Host's Subscription Fee is invoiced under Section 2.2 on Platform Payout — after the Booking Site's fees, and excluding cleaning fees and taxes. Host does not take fifteen percent (15%) of the platform's own fee.

1.1.7 Reporting. Host agrees to prepare monthly reports to Subscriber detailing Platform Payout (the base for the Subscription Fee), Booking Site deductions, and costs and expenses incurred by Host on Subscriber's behalf. Subscriber will be provided with access to those reports by email or file-sharing folder. Those reports will be made available, with respect to each month, on or before the 15th day of the next succeeding month.
...
2.2 Subscription Fee. The Subscription Fee is fifteen percent (15%) of Platform Payout. "Platform Payout" is defined in Section 1.1.6: the amount the Booking Site deposits to Subscriber after the Booking Site deducts its own charges (including host service fees and similar platform charges). The Subscription Fee is calculated after those Booking Site / platform fees are taken, not on the guest-facing fare before the platform cut.

If the Booking Site's deposit includes a cleaning fee or taxes, those amounts are subtracted before applying the fifteen percent (15%). Host does not charge the Subscription Fee on the cleaning fee. Extra person fees, weekly discounts as they affect the Accommodation fare, and any markup or markdown Host applies to the Accommodation fare are part of Platform Payout only to the extent they are included in that deposit. Platform Payout for this calculation also does not include other fees allocated one hundred percent (100%) to Host under the parties' PMS commission settings (including, without limitation, early check-in fees and Electronics Fees).

The Subscription Fee is calculated and recognized at check-out only when Platform Payout is actually received. On canceled reservations where no Platform Payout is received, Host is not entitled to any Subscription Fee.

All refunds, discounts, or credits issued to Guests are the sole financial responsibility of Subscriber and shall not reduce Platform Payout for purposes of calculating the Subscription Fee. Subscriber shall reimburse Host for any such amounts within ten (10) days.
