...
export default function GuidebookPage({ params, searchParams: spPromise }: {
  ...
  const tabBtn = (id: string, svg: string, label: string) => (
    <button key={id} onClick={() => { setTab(id); contentRef.current?.scrollTo(0,0); }}
      className={`flex-shrink-0 w-14 flex flex-col items-center justify-center gap-0.5 border-none font-inherit cursor-pointer transition-colors`}
      style={{ color: tab === id ? T.accentColor : '#8b7355', fontFamily: 'inherit' }}>
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">{svg.includes('<') ? <g dangerouslySetInnerHTML={{ __html: svg }} /> : <path d={svg} />}</svg>
      <span className="text-[7px] font-semibold tracking-wider whitespace-nowrap">{label}</span>
    </button>
  );

  const sectionTitle = (emoji: string, text: string) => (
    <h2 className="font-serif text-base text-stone-800 px-3.5 pt-3 pb-1.5" style={{ fontFamily: "'DM Serif Display', serif" }}>
      <span className="mr-1">{emoji}</span>{text}
    </h2>
  );

  const card = (icon: string, title: string, meta: string, tag?: [string, string], desc?: string) => (
    <div className="bg-white rounded-lg px-3 py-2.5 mx-3.5 mb-1 border border-stone-100">
      <div className="flex items-center gap-2">
        <span className="text-xl flex-shrink-0">{icon}</span>
        <div className="flex-1">
          <div className="text-[11px] font-bold text-stone-800">{title}</div>
          <div className="text-[9px] text-stone-400 mt-0.5">{meta}</div>
        </div>
        {tag && <span className={`text-[7px] font-semibold px-1.5 py-0.5 rounded whitespace-nowrap flex-shrink-0 ${tag[0] === 'r' ? 'bg-amber-50 text-amber-800' : tag[0] === 'b' ? 'bg-blue-50 text-blue-800' : tag[0] === 'g' ? 'bg-green-50 text-green-800' : 'bg-stone-100 text-stone-600'}`}>{tag[1]}</span>}
      </div>
      {desc && <div className="text-[9px] text-stone-400 mt-1.5 ml-9 leading-relaxed">{desc}</div>}
    </div>
  );

  const linkCard = (href: string, icon: string, title: string, meta: string, tag?: [string, string], desc?: string) => (
    <a href={href} target="_blank" rel="noopener" className="block bg-white rounded-lg px-3 py-2.5 mx-3.5 mb-1 border border-stone-100 hover:shadow-sm transition-shadow no-underline text-inherit">
      <div className="flex items-center gap-2">
        <span className="text-xl flex-shrink-0">{icon}</span>
        <div className="flex-1">
          <div className="text-[11px] font-bold text-stone-800">{title}</div>
        ...
        {tag && <span className={`text-[7px] font-semibold px-1.5 py-0.5 rounded whitespace-nowrap flex-shrink-0 ${tag[0] === 'r' ? 'bg-red-50 text-red-800' : tag[0] === 'b' ? 'bg-blue-50 text-blue-800' : tag[0] === 'g' ? 'bg-green-50 text-green-800' : tag[0] === 'p' ? 'bg-purple-50 text-purple-800' : 'bg-amber-50 text-amber-800'}`}>{tag[1]}</span>}
      </div>
      {desc && <div className="text-[9px] text-stone-400 mt-1.5 ml-9 leading-relaxed">{desc}</div>}
    </a>
  );
    ...
    <div className="bg-white rounded-lg mb-1 border border-stone-100 overflow-hidden">
      <details className="group">
        <summary className="flex items-center gap-2 px-3 py-2.5 cursor-pointer list-none">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm flex-shrink-0" style={{ background: iconBg }}>{icon}</div>
          <div className="flex-1">
            <div className="text-[11px] font-bold text-stone-800">{title}</div>
            <div className="text-[9px] text-stone-400 mt-0.5">{desc}</div>
          </div>
          <span className="text-stone-300 text-xs transition-transform group-open:rotate-90">›</span>
        </summary>
        {body && <div className="px-3 pb-2.5 ml-9 text-[10px] text-stone-600 leading-relaxed border-t border-stone-50 pt-2"><div dangerouslySetInnerHTML={{ __html: body }} /></div>}
      </details>
    </div>
  );

  const btn = (href: string, label: string, style: string) => (
    <a href={href} target="_blank" rel="noopener" className={`text-[9px] font-semibold px-3 py-1.5 rounded-md no-underline inline-block ${style}`}>{label}</a>
  );
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col max-w-md mx-auto relative" style={{ fontFamily: "'Plus Jakarta Sans', -apple-system, sans-serif" }}>
      ...
      <div className="flex items-center gap-2 px-3.5 pb-2.5 flex-shrink-0" style={{ background: '#2c1810', paddingTop: '8px' }}>
        <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[11px] font-extrabold flex-shrink-0" style={{ background: 'linear-gradient(135deg,#f5c842,#e8b832)', color: '#2c1810' }}>SV</div>
        <div className="flex-1 text-center">
          <div className="text-sm font-bold tracking-wider" style={{ color: '#f5c842' }}>Summers Vacations</div>
          <div className="text-[8px]" style={{ color: '#8b7355' }}>Branson, Missouri</div>
        </div>
        <div className="text-[8px] text-right" style={{ color: '#8b7355' }}>{mode === 'guest' ? 'July 6, 2026' : 'Checked out'}</div>
      </div>

      {/* Mode Toggle */}
      <div className="flex gap-1.5 px-3.5 py-1 flex-shrink-0 justify-center" style={{ background: '#3d2b1f' }}>
        <button onClick={() => setMode('guest')} className={`text-[8px] font-semibold px-3 py-1 rounded-full border-none cursor-pointer font-inherit transition-colors ${mode === 'guest' ? 'text-stone-900' : 'text-stone-400'}`}
          style={{ background: mode === 'guest' ? T.accentColor : 'rgba(255,255,255,.1)' }}>🏠 Guest Mode</button>
        <button onClick={() => setMode('branson')} className={`text-[8px] font-semibold px-3 py-1 rounded-full border-none cursor-pointer font-inherit transition-colors ${mode === 'branson' ? 'text-stone-900' : 'text-stone-400'}`}
      ...
      <div className="px-3.5 py-1.5 flex items-center gap-2 flex-shrink-0" style={styles.bar}>
        ...
        <span className="text-[10px] font-semibold text-white flex-1">{T.name}</span>
        <a href={T.featuredLink} target="_blank" rel="noopener" className="text-[8px] font-semibold px-2 py-0.5 rounded-full no-underline" style={{ background: T.accentColor, color: '#2c1810' }}>See Event</a>
      </div>
      ...
      <div ref={contentRef} className="flex-1 overflow-y-auto scroll-smooth pb-2" style={{ scrollbarWidth: 'none' }}
          ...
          <>
            {mode === 'guest' ? (
              <>
                <div className="px-4 pt-3 pb-4" style={styles.bar}>
                  <h1 className="font-serif text-xl" style={{ fontFamily: "'DM Serif Display', serif", color: '#f5c842' }}>
                    {guestName ? `Welcome, ${guestName}!` : `Welcome to ${prop.name}!`} 🤠
                  </h1>
                  <div className="text-[11px] mt-0.5" style={{ color: '#c4b5a0' }}>{prop.name} • Check-in at {prop.checkIn.time}</div>
                </div>
                <div className="mx-3.5 mt-[-10px] relative z-10 rounded-xl px-3.5 py-3 flex items-center gap-2.5 shadow-lg" style={{ background: 'linear-gradient(135deg,#f5c842,#e8b832)' }}>
                  <div>
                    <div className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: '#5c3d1e' }}>Your Door Code</div>
                    <div className="text-2xl font-extrabold tracking-[4px]" style={{ color: '#2c1810' }}>{guestCode || prop.checkIn.doorCode}</div>
                  </div>
                  <span className="ml-auto text-[9px] font-semibold px-2 py-1 rounded-full whitespace-nowrap" style={{ background: 'rgba(44,24,16,.15)', color: '#5c3d1e' }}>✦ Ready</span>
                </div>
              </>
            ) : (
              <div className="px-4 pt-3 pb-4" style={{ background: 'linear-gradient(135deg,#1e3a5f,#2d5a8e)' }}>
                <h1 className="font-serif text-xl text-amber-300" style={{ fontFamily: "'DM Serif Display', serif" }}>Welcome back to Branson! 🎣</h1>
                <div className="text-[11px] mt-0.5 text-blue-200">{T.tagline}</div>
              </div>
            )}

            {/* Weather & Quick Info */}
            <div className="mx-3.5 mt-2.5 bg-white rounded-xl px-3 py-2.5 border border-stone-100 flex items-center gap-2">
              <span className="text-2xl font-bold text-stone-800">87°</span>
              <div className="flex-1"><div className="text-[10px] font-semibold text-stone-800">Branson, MO</div><div className="text-[9px] text-stone-400">☀️ Sunny</div></div>
              <div className="flex gap-2"><div className="text-center text-[9px] text-stone-400"><div className="font-semibold text-stone-600">Wed</div>☀️ 89°</div><div className="text-center text-[9px] text-stone-400"><div className="font-semibold text-stone-600">Thu</div>⛅ 86°</div></div>
            </div>

            <div className="flex gap-1.5 px-3.5 pt-1.5">
              <div className="flex-1 bg-white rounded-lg px-2.5 py-2 text-center border border-stone-100"><div className="text-[8px] uppercase tracking-wide text-stone-400 font-semibold">WiFi</div><div className="text-[10px] font-bold text-stone-800 mt-0.5">{prop.wifi.network}</div></div>
              <div className="flex-1 bg-white rounded-lg px-2.5 py-2 text-center border border-stone-100"><div className="text-[8px] uppercase tracking-wide text-stone-400 font-semibold">Check-out</div><div className="text-[11px] font-bold text-stone-800 mt-0.5">10 AM</div></div>
              <div className="flex-1 bg-white rounded-lg px-2.5 py-2 text-center border border-stone-100"><div className="text-[8px] uppercase tracking-wide text-stone-400 font-semibold">Pool</div><div className="text-[11px] font-bold text-stone-800 mt-0.5">8–10PM</div></div>
            </div>

            {mode === 'branson' && (
              <div className="mx-3.5 mt-2 bg-green-50 rounded-lg px-3 py-2.5 border border-green-200">
                <div className="flex items-center gap-2"><span className="text-lg">🌲</span><div><div className="text-[11px] font-bold text-green-700">Your stay has ended</div><div className="text-[9px] text-green-600">But Branson is still here! Check shows, fishing, golf, and dining below.</div></div></div>
              </div>
            )}

            {/* Seasonal Feature */}
            <div className="mx-3.5 mt-2 rounded-xl px-3.5 py-3" style={styles.bar}>
              <div className="flex items-center gap-2"><span className="text-lg">{T.emoji}</span><h3 className="text-sm font-bold text-white flex-1">{T.featuredEvent}</h3></div>
              <div className="flex gap-1.5 mt-2">
                <a href={T.featuredLink} target="_blank" rel="noopener" className="text-[9px] font-semibold px-3 py-1.5 rounded-md no-underline" style={{ background: T.accentColor, color: '#2c1810' }}>🎫 Learn More</a>
                <span className="text-[9px] text-white/70 self-center">{T.sdcEvent}</span>
              </div>
            </div>

            {/* Push Message */}
            <div className="mx-3.5 mt-1.5 mb-1 bg-amber-50 rounded-lg px-3 py-2 border border-amber-200">
              <div className="text-[9px] text-amber-800 font-semibold">💡 {T.pushMessage}</div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-1.5 px-3.5">
              {[{e:'🎆',t:'Fireworks',d:'SDC Night Sky',l:'https://www.silverdollarcity.com/'},{e:'🎸',t:'Bohemian Queen',d:'Freddie Tribute',l:'https://www.claycooper.com/'},{e:'🎬',t:'Free Movies',d:'Last 2 weeks!',l:'https://www.bransonimax.com/'},{e:'⛳',t:'Golf Camp',d:'Ozarks National',l:'https://bigcedar.com/golf/ozarks-national/'}].map((x,i) => (
                <a key={i} href={x.l} target="_blank" rel="noopener" className="block bg-white rounded-lg px-2.5 py-2 border border-stone-100 no-underline text-inherit"><div className="text-lg">{x.e}</div><div className="text-[11px] font-bold text-stone-800 mt-0.5">{x.t}</div><div className="text-[9px] text-stone-400">{x.d}</div></a>
              ))}
            </div>
            ...
            {[{t:'7PM',n:'Grand Jubilee',v:'Grand Country',l:'https://www.grandcountrymusichall.com/'},{t:'8PM',n:'Bohemian Queen',v:'Clay Cooper',l:'https://www.claycooper.com/'},{t:'8PM',n:'The Haygoods',v:'Haygood Theater',l:'https://www.haygoodshow.com/'}].map((x,i) => (
              <a key={i} href={x.l} target="_blank" rel="noopener" className="flex items-center gap-2 mx-3.5 mb-1 bg-white rounded-lg px-2.5 py-2 border border-stone-100 no-underline text-inherit">
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded whitespace-nowrap" style={{ background: '#2c1810', color: '#f5c842' }}>{x.t}</span>
                <span className="text-[10px] font-semibold flex-1 text-stone-800">{x.n}</span>
                <span className="text-[8px] text-stone-400">{x.v}</span>
              </a>
            ))}
            ...
            <a href="https://notchcondos.guestywebsites.com/" target="_blank" rel="noopener" className="block mx-3.5 my-2 rounded-xl px-3.5 py-3 flex items-center gap-2 no-underline" style={{ background: 'linear-gradient(135deg,#f5c842,#e8b832)' }}>
              ...
              <div className="flex-1"><div className="text-xs font-bold text-stone-800">Book Your Next Stay</div><div className="text-[9px] text-amber-800">Direct booking • Best rates • Summers Vacations</div></div>
          ...
          <>
            {mode === 'guest' ? (
              <>
                <div className="px-4 pt-3 pb-3" style={{ background: 'linear-gradient(135deg,#3d2b1f,#5c4033)', color: 'white', overflow: 'hidden', position: 'relative' }}>
                  ...
                  <h1 className="font-serif text-xl" style={{ fontFamily: "'DM Serif Display', serif" }}>{prop.name} Guidebook</h1>
                  <p className="text-[10px] mt-0.5" style={{ color: '#c4b5a0' }}>Your complete house manual</p>
                </div>
                <div className="px-3.5 pt-2">
                  ...
                  <div className="bg-green-50 rounded-lg mb-1 border border-green-200 overflow-hidden">
                    <details className="group">
                      <summary className="flex items-center gap-2 px-3 py-2.5 cursor-pointer list-none">
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm flex-shrink-0" style={{ background: '#bbf7d0' }}>🧹</div>
                        <div className="flex-1"><div className="text-[11px] font-bold text-green-700">Cleanliness Status</div><div className="text-[9px] text-green-600">✓ Inspected & ready!</div></div>
                        <span className="text-green-400 text-xs transition-transform group-open:rotate-90">›</span>
                      </summary>
                      <div className="px-3 pb-2.5 ml-9 text-[10px] text-green-700 leading-relaxed border-t border-green-100 pt-2">Unit cleaned, sanitized, and inspected. If anything needs attention, text Brian.</div>
                    </details>
                  </div>
                </div>
              </>
              ...
              <>
                <div className="px-4 pt-3 pb-3" style={{ background: T.gradient, color: 'white', overflow: 'hidden', position: 'relative' }}>
                  <div style={{ position: 'absolute', right: 10, bottom: -8, fontSize: 60, opacity: 0.08 }}>🏡</div>
                  <h1 className="font-serif text-xl" style={{ fontFamily: "'DM Serif Display', serif" }}>Our Properties</h1>
                  <p className="text-[10px] mt-0.5" style={{ color: '#c4b5a0' }}>Browse all Summers Vacations rentals</p>
                </div>
                <div className="px-3.5 pt-2">
                  <div className="bg-green-50 rounded-lg px-3 py-2.5 mb-2 border border-green-200">
                    <div className="flex items-center gap-2"><span className="text-lg">🌲</span><div><div className="text-[11px] font-bold text-green-700">Your stay at {prop.name} has ended</div><div className="text-[9px] text-green-600">But there are 6 more properties to explore!</div></div></div>
                  </div>
                  ...
                  {[
                    ...
                    <a key={i} href="https://notchcondos.guestywebsites.com/" target="_blank" rel="noopener" className={`block bg-white rounded-lg px-3 py-2.5 mb-1 border no-underline text-inherit ${i===0?'border-l-4 border-amber-400':''}`}>
                      <div className="flex items-center gap-2"><span className="text-2xl">{p.icon}</span><div className="flex-1"><div className="text-[11px] font-bold text-stone-800">{p.name}</div><div className="text-[9px] text-stone-400">{p.meta}</div></div>
                        {i>0 && <span className="text-[7px] font-semibold px-1.5 py-0.5 rounded bg-green-50 text-green-700 whitespace-nowrap">Book Now</span>}</div>
                      <div className="text-[8px] text-green-700 font-semibold mt-1 ml-9">{p.desc}</div>
                    </a>
                  ))}

                  <div className="bg-amber-50 rounded-lg px-3 py-2.5 my-2 border border-amber-200">
                    <div className="text-[10px] font-bold text-amber-800">💡 Bundle & Save</div>
                    <div className="text-[9px] text-amber-700 mt-1">Planning a family reunion or group trip? Book <strong>2+ units</strong> and save! All properties are in the same complex. Contact Brian for custom multi-unit pricing: <strong>314-565-0589</strong></div>
                  </div>
                </div>
              </>
            )}
            ...
            {mode === 'guest' && (
              <div className="px-3.5">
                <div className="font-serif text-sm text-stone-800 pt-2 pb-1" style={{ fontFamily: "'DM Serif Display', serif" }}>🎬 Video Guides</div>
                {[
                  ...
                  <a key={i} href={u} target="_blank" rel="noopener" className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 mb-0.5 border border-stone-100 no-underline text-inherit">
                    <span className="text-sm">▶</span><span className="text-[10px] font-semibold text-stone-700">{t}</span>
                  </a>
                ))}
              </div>
            )}
            ...
            <a href="https://notchcondos.guestywebsites.com/" target="_blank" rel="noopener" className="block mx-3.5 my-2 rounded-xl px-3.5 py-3 flex items-center gap-2 no-underline" style={{ background: 'linear-gradient(135deg,#f5c842,#e8b832)' }}>
              <span className="text-2xl">🏠</span><div className="flex-1"><div className="text-xs font-bold text-stone-800">Book Your Next Stay</div><div className="text-[9px] text-amber-800">Direct booking • Best rates</div></div><span className="text-lg text-amber-800">›</span>
            </a>
          </>
        )}
        ...
        {tab === 'adventure' && (
          <>
            <div className="px-4 pt-3 pb-3" style={styles.bar}><h1 className="font-serif text-xl text-white" style={{ fontFamily: "'DM Serif Display', serif" }}>Branson Adventures</h1><p className="text-[10px] mt-0.5 text-amber-200">Thrills • Parks • Outdoors</p></div>
            {sectionTitle('⭐','Featured — Silver Dollar City')}
            <div className="mx-3.5 mb-1.5 rounded-xl px-3.5 py-3" style={styles.bar}>
              <div className="flex items-center gap-1.5"><span className="text-lg">🎢</span><h3 className="text-sm font-bold text-white">Silver Dollar City</h3></div>
              <div className="text-[10px] mt-1 text-amber-200 leading-relaxed">America&apos;s #1 Theme Park! Outlaw Run, Time Traveler, Powder Keg, Mystic River Falls. 12 min drive.</div>
              <div className="flex gap-1.5 mt-2">
                <a href="https://www.silverdollarcity.com/" target="_blank" rel="noopener" className="text-[9px] font-semibold px-3 py-1.5 rounded-md no-underline" style={{ background: T.accentColor, color: '#2c1810' }}>🎫 Tickets</a>
                <a href="https://www.silverdollarcity.com/theme-park/attractions/rides/" target="_blank" rel="noopener" className="text-[9px] font-semibold px-3 py-1.5 rounded-md no-underline bg-white/20 text-white">🎢 Rides</a>
              </div>
            </div>
        ...
        {tab === 'fishing' && (
          <>
            <div className="px-4 pt-3 pb-3" style={{ background: 'linear-gradient(135deg,#1e3a5f,#2d5a8e)' }}><h1 className="font-serif text-xl text-white" style={{ fontFamily: "'DM Serif Display', serif" }}>Branson Fishing Guide</h1><p className="text-[10px] mt-0.5 text-blue-300">Spots • Guides • Tackle • Report</p>
              <span className="inline-block text-[9px] mt-1 px-2 py-0.5 rounded-full bg-white/20 text-white">🎣 July 6, 2026</span>
            </div>
            ...
            <div className="mx-3.5 mb-1.5 rounded-xl px-3.5 py-3" style={{ background: 'linear-gradient(135deg,#1e3a5f,#2d5a8e)' }}>
              <div className="flex items-center gap-1.5"><span className="text-base">🟢</span><h3 className="text-sm font-bold text-white">Bite of the Day</h3></div>
              <div className="text-[10px] mt-1 text-blue-200 leading-relaxed">Taneycomo trout biting strong on pink worms below Fall Creek. Table Rock bass on topwater at dawn. Walleye excellent — 30-50 fish per trip.</div>
              <div className="flex gap-2 mt-1 flex-wrap"><span className="text-[9px]"><strong className="text-amber-300">Table Rock</strong> 77–80°F</span><span className="text-[9px]"><strong className="text-amber-300">Taneycomo</strong> 50–55°F</span></div>
            </div>
            ...
            <div className="mx-3.5 my-2 bg-amber-50 rounded-lg px-3 py-2 border border-amber-200">
              <div className="text-[9px] font-bold text-amber-800">🎫 Missouri fishing license required. Get one at any tackle shop or mdc.mo.gov</div>
            </div>
          </>
        )}
        ...
        {tab === 'shows' && (
          <>
            <div className="px-4 pt-3 pb-3" style={{ background: 'linear-gradient(135deg,#7c3aed,#6d28d9)' }}><h1 className="font-serif text-xl text-white" style={{ fontFamily: "'DM Serif Display', serif" }}>Branson Shows</h1><p className="text-[10px] mt-0.5 text-purple-300">Music • Comedy • Dinner • Family</p>
              <span className="inline-block text-[9px] mt-1 px-2 py-0.5 rounded-full bg-white/20 text-white">🎭 July 6, 2026</span>
            </div>
            ...
            <div className="mx-3.5 my-2 bg-amber-50 rounded-lg px-3 py-2 border border-amber-200">
              <div className="text-[9px] text-amber-800 font-semibold">🏆 World Cup Watch Parties at Big Cedar Lodge — daily through Jul 19</div>
            </div>
          </>
        )}
        ...
        {tab === 'golf' && (
          <>
            <div className="px-4 pt-3 pb-3" style={{ background: 'linear-gradient(135deg,#2a5e3e,#1e4a30)' }}><h1 className="font-serif text-xl text-white" style={{ fontFamily: "'DM Serif Display', serif" }}>Branson Golf Guide</h1><p className="text-[10px] mt-0.5 text-green-300">Spotlight • Rates • Tee times</p><span className="inline-block text-[9px] mt-1 px-2 py-0.5 rounded-full bg-white/20 text-white">⛳ July 6, 2026</span></div>
            ...
            <div className="mx-3.5 rounded-xl px-3.5 py-3" style={{ background: 'linear-gradient(135deg,#166534,#15803d)' }}>
              <div className="flex items-center gap-1.5"><span className="text-lg">🏆</span><h3 className="text-sm font-bold text-white">Branson Hills Golf Club</h3></div>
              <div className="text-[10px] mt-1 text-green-200 leading-relaxed">#1 Public in MO (Golfpass 2026). Ranked #10 Public in the U.S.! Dramatic 130-ft elevation changes. Missouri golf history museum.</div>
              <div className="text-[9px] mt-1 text-amber-300 px-2 py-1 rounded bg-black/20 leading-relaxed">🤯 Fun Fact: The first tee drops 130+ ft — that&apos;s a 13-story building!</div>
              <div className="flex gap-2 mt-1 flex-wrap"><span className="text-[9px]"><strong className="text-amber-300">Rates</strong> $235–$265</span><span className="text-[9px]"><strong className="text-amber-300">15 min</strong></span><span className="text-[9px]"><strong className="text-amber-300">Par 72</strong> 7,000 yds</span></div>
              <div className="flex gap-1.5 mt-2">
                <a href="https://bransonhillsgolfclub.com/" target="_blank" rel="noopener" className="text-[9px] font-semibold px-3 py-1.5 rounded-md no-underline" style={{ background: '#f5c842', color: '#2c1810' }}>📅 Book Tee Time</a>
                <a href="https://bransonhillsgolfclub.com/rates/" target="_blank" rel="noopener" className="text-[9px] font-semibold px-3 py-1.5 rounded-md no-underline bg-white/20 text-white">💰 Rates</a>
              </div>
            </div>
            ...
            <div className="mx-3.5 my-2 bg-amber-50 rounded-lg px-3 py-2 border border-amber-200">
              <div className="text-[9px] text-amber-800 font-semibold">💡 Weekday tee times save $50-100+. Book 30 days ahead for Big Cedar courses.</div>
            </div>
          </>
        )}
        ...
        {tab === 'dining' && (
          <>
            <div className="px-4 pt-3 pb-3" style={{ background: 'linear-gradient(135deg,#be123c,#e11d48)' }}><h1 className="font-serif text-xl text-white" style={{ fontFamily: "'DM Serif Display', serif" }}>Branson Dining</h1><p className="text-[10px] mt-0.5 text-red-300">Restaurants • Bars • Waterfront</p></div>
